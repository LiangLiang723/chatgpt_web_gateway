...
长期仓库任务优先维持一个主开发会话。不要因为需要“继续”、页面刷新或一次工具失败就主动创建平行 ChatGPT 会话；项目恢复必须依赖 Git + Project Memory，而不是依赖对话列表能否访问。
...
→ AGENTS.md
→ PROJECT_STATE.md
→ Active Plan / NEXT_TASK
→ 检查未提交 diff
→ 从最小可执行 checkpoint 继续
```

如果平台频率保护、对话记录限制、工具硬错误或会话长度迫使当前执行中断：

1. 不创建多个平行会话尝试绕过限制。
2. 停止会产生外部副作用的真实 E2E / 发布操作。
3. 如果 Phase、NEXT_TASK、blocker 或 Plan 状态已经变化，先把事实写回仓库。
4. 已达到独立验收点的改动优先验证并提交；尚未达到提交点的工作保留真实 working tree，不伪造完成状态。
5. 新会话只按上面的仓库恢复入口继续，不要求读取旧聊天记录。
...
不要让大量已经验证的任务长期只存在于未提交工作树或聊天上下文；独立 checkpoint 能显著降低换会话、频率限制或工具故障后的恢复成本。
