...
ACTIVE_PLAN=none
NEXT_TASK=review-legacy-conversation-executor-dead-code
...
- **当前状态：** `v1-acceptance-complete`。当前公开版本为 **V0.1.5**：V0.1.4 已关闭大型多行 Composer transport、bounded Browser diagnostics、active `/v1/diagnostics` 与 Docker Host proxy passthrough；V0.1.5 进一步关闭真实 Pi 同 session 网页 Conversation 连续性缺陷。成功的 anonymous FRESH 从首轮开始按 generated persisted Conversation id 保留 bounded Page affinity，同 runtime 的唯一 full-history candidate 因而直接在原 Page APPEND；客户端可见 `conversationKey` 仍保持 `undefined`，匿名匹配、歧义 FRESH、queue 锁后重验证与 restart RESTORE 语义不变。continuity/fixture focused **3 files / 26 tests**、merged `main` fresh full **91 files / 640 tests** 与真实 Pi `0.84.4` 两轮 + distractor Page gate 均已通过。V0.1.5 发布同名 Git Tag / GitHub Release；Docker registry image 未发布。
...
- **最新 maintenance patch：** 2026-09-01 针对“真实 Pi 同一 session 每轮在网页端新开 Conversation”的续接问题，Conversation Engine 让成功的 anonymous FRESH 从首轮开始按 generated persisted Conversation id 保留 bounded Page affinity；同 runtime 的唯一 full-history candidate 因而可在原 Page APPEND，不再为了建立 identity 先跨 Page/URL RESTORE。`conversationKey` 仍保持 `undefined`，匿名历史唯一性、歧义 FRESH、内部 queue 锁后重验证与 restart RESTORE 语义不变。真实 Pi `0.84.4` 本地协议捕获确认同 session 会重发完整 history、system/tools 稳定，且 Pi 不发送可用的稳定 session header。使用 `http://192.168.3.83:7890` 与新隔离登录 Profile 的真实两轮 Pi Web gate 已通过：同一 Pi session 连续两次请求、首轮后占用 distractor Page、`gatewayRequests=2`、首轮 Browser Prompt **21,019 UTF-8 bytes** / 16 tools、第二轮 `samePageContinuation=true`。该修复与验收证据现已 fast-forward 进入 `main`。
- **最新 deterministic / Docker 基线：** `release/v0.1.5` 版本元数据完成后 fresh `corepack pnpm verify` 通过 **91 test files / 640 tests**，format/lint/typecheck/build/Project Memory/Docs/Architecture/Version 全绿；continuity/fixture focused coverage 为 **3 files / 26 tests**，fast-forward 后 merged `main` 的 continuity candidate 此前同样为 **91/640**。V0.1.4 release branch 先前为 **91/638**。代表性 `docker compose config` 已确认 `host.docker.internal=host-gateway` 与 `CHATGPT_PROXY_SERVER` / `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` / `NO_PROXY` passthrough。最近一次完整 `linux/amd64` Docker 仍是 V0.1.0 release candidate image `sha256:866e2b280a1a3ab790c1ab4ae725ec0c1fe345420b7aeec438497806fbd896fa` + full `docker:smoke` PASS；本 continuity PATCH 不改 Dockerfile、依赖或 migration，未重复 Docker image build/smoke。
- **最新真实网页事实：** 2026-09-01 使用 `http://192.168.3.83:7890` 与新隔离登录 Profile 的 fresh `inspect:chatgpt` 得到 `auth=authenticated` / Composer unique；空 Composer 下 Send control 未挂载。随后服务器实际安装的 Pi `0.84.4` 完成同一 session 两轮 `Pi → Gateway → ChatGPT Web` continuity gate：首轮精确声明 16 tools、最终 Browser Prompt **21,019 UTF-8 bytes**，首轮后 harness 占用 distractor Page，第二轮仍复用首轮 anonymous Conversation Page，结果为 `gatewayRequests=2` / `samePageContinuation=true`。首次 real gate 的第二轮随机 token 被模型少抄最后两个字符，因此在结构断言前失败；按真实 E2E 退避规则仅允许的一次立即复现随后完整通过。此前本地真实 Pi 协议捕获还确认同 session 第二轮为完整 `system → user → assistant → user` history，tool round-trip 为 `assistant(content:null,tool_calls) → tool(tool_call_id)`，system/tool definitions 跨请求稳定，且 HTTP headers 没有 Pi session id。
...
6. ✅ Git / release closure：V1 feature branch 已 fast-forward 合并到 `main`；`V0.1.0` 与当前 `V0.1.5` 发布元数据均同步 `VERSION` / `package.json` / `CHANGELOG` / Project State，并创建同名 Git Tag / GitHub Release。Docker registry image 未发布。
...
- 2026-09-01：**V0.1.5** 发布关闭 Pi same-session Web Conversation continuity maintenance：anonymous FRESH 成功后不再立即丢弃 Page affinity，而是绑定 generated persisted Conversation id；同 runtime 下一轮唯一 full-history request 可直接复用原 Page。真实 Pi `0.84.4` 本地捕获确认完整 history/system/tools/header 行为；新隔离 Profile 经 `inspect:chatgpt` 确认 authenticated 后，使用 `http://192.168.3.83:7890` 的两轮 same-session + distractor Page E2E 完整通过：**21,019-byte / 16 tools / gatewayRequests=2 / samePageContinuation=true**。fast-forward 后 merged `main` fresh deterministic **91/640** 全绿；V0.1.5 创建同名 Git Tag / GitHub Release，Docker Registry 镜像未发布。
...
- 当前 Pi continuity fix 已通过真实 Pi 本地协议捕获、deterministic same-Page regressions，以及新的“两轮同 session + distractor Page” authenticated ChatGPT Web E2E；当前剩余风险主要回到 ChatGPT DOM、认证与模型输出本身的外部不稳定性。首次 continuity gate 曾出现一次随机 token 少抄最后两个字符的模型输出偏差，唯一一次允许的立即复现随后通过全部结构断言。
