...
- **最新 deterministic / Docker 基线：** fast-forward 后 merged `main` fresh `corepack pnpm verify` 通过 **91 test files / 640 tests**，format/lint/typecheck/build/Project Memory/Docs/Architecture/Version 全绿；其中 continuity/fixture focused coverage 为 **3 files / 26 tests**。V0.1.4 release branch 先前为 **91/638**。代表性 `docker compose config` 已确认 `host.docker.internal=host-gateway` 与 `CHATGPT_PROXY_SERVER` / `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` / `NO_PROXY` passthrough。最近一次完整 `linux/amd64` Docker 仍是 V0.1.0 release candidate image `sha256:866e2b280a1a3ab790c1ab4ae725ec0c1fe345420b7aeec438497806fbd896fa` + full `docker:smoke` PASS；本 continuity PATCH 不改 Dockerfile、依赖或 migration，未重复 Docker image build/smoke。
...
- 2026-09-01：Pi same-session Web Conversation continuity maintenance 完成实现、deterministic、authenticated real-Web gate 与 `main` fast-forward 集成：anonymous FRESH 成功后不再立即丢弃 Page affinity，而是绑定 generated persisted Conversation id；同 runtime 下一轮唯一 full-history request 可直接复用原 Page。真实 Pi `0.84.4` 本地捕获确认完整 history/system/tools/header 行为；新隔离 Profile 经 `inspect:chatgpt` 确认 authenticated 后，使用 `http://192.168.3.83:7890` 的两轮 same-session + distractor Page E2E 完整通过：**21,019-byte / 16 tools / gatewayRequests=2 / samePageContinuation=true**。fast-forward 后 merged `main` fresh deterministic **91/640** 全绿。未改版本、未发布 Docker/Release。
